...
class MailerService {
  ...
  public static async orderSuccess (email: string, order: any) {
    const mailerOptions: Mail.Options = {
      to: email,
      subject: '[Hệ thống] Xác thực email',
    };
    ...
    await this.sendMail(mailerOptions, 'orderSuccess', templateArgs);
  }

  public static async exportOrder (email: string, order: any) {
    const mailerOptions: Mail.Options = {
      to: email,
      subject: '[Hệ thống] Xác thực email',
    };
    ...
    await this.sendMail(mailerOptions, 'exportOrder', templateArgs);
  }
