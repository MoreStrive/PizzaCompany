    ...
    <div :class="`w-full 'bg-white' : 'bg-[#18191a]' px-3 xl:px-0 overflow-auto pb-4`">
        <div class="grid grid-cols-12 gap-4">
            <div class="col-span-12 xl:col-span-6 p-3 rounded-md bg-white">
                ...
                <div class=" text-lg">
                    <div class="mb-2">
                        <strong>Tên người nhận:</strong> {{ order.shipping_name || '--' }}
                    </div>
                    <div class="mb-2">
                        <strong>Số điện thoại người nhận:</strong> {{ order.shipping_phone || '--' }}
                    </div>
                    <div class="mb-2">
                        <strong>Email người nhận:</strong> {{ order.shipping_email || '--' }}
                    </div>
                    <div class="mb-2">
                        <strong>Địa chỉ người nhận:</strong> {{ order.shipping_address || '--' }}
                    </div>
                    <div>
            ...
            <div class="col-span-12 xl:col-span-6">
                <div class="p-3">
                    ...
                    <div class="mt-3 grid grid-cols-1 gap-2 text-lg">
                        <div>
                            <strong>Thời gian tạo:</strong> {{ order.created_at | dateFormat('HH:mm dd/MM/yyyy') }}
                        </div>
                        <div>
                            <strong>Phương thức:</strong> {{ order.payment_method === 'cash' ? 'Thanh toán tiền mặt' : 'Thanh toán Online' }}
                        </div>
                        <div>
                        ...
                        <div v-if="order.payment_confirmation" class="mt-3">
                            <strong>Chứng thực:</strong>
                            <div class="mt-2">
                                <img class="max-w-[300px] h-auto object-cover" :src="order.payment_confirmation || ''" alt="">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                if (this.currentUser && this.currentUser.customer_class) {
                    switch (this.currentUser.customer_class) {
                        ...
                        case 'Diamond':
                            ...
                            break;
                        default:
                            customerClassPrice = this.order.total_price;
                    }
                }
